...
function setupMultiTouch() {
    ...
    // Обработка мультитач-событий для элементов задач
    ...
    // Обработка мультитач-событий для чекбоксов
    checkboxes.forEach(checkbox => {
    ...
    // Обработка мультитач-событий для карточки задания
    if (taskCard) {
        taskCard.addEventListener('touchstart', handleMultiTouchStart, { passive: false });
        taskCard.addEventListener('touchend', handleMultiTouchEnd, { passive: false });
    }
}
