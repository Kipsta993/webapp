document.addEventListener('DOMContentLoaded', function() {
    ...
    // Обработчик для применения пользовательской темы
    ...
    resetStreakButton.addEventListener('click', resetStreak);
});
...
function resetStreak() {
    // Сохраняем текущую серию как максимальную, если она больше
    ...
    // Сбрасываем серию дней до начального значения
    const streakCountElement = document.getElementById('streak-count');
    streakCountElement.textContent = '1';
    localStorage.setItem('streak', 1);
    
    // Показываем уведомление об успешном сбросе
    alert('Серия дней сброшена до начального значения!');
}
