...
(function() {
  ...
  function startActivityTracking() {
    ...
    trackingInterval = setInterval(function() {
      ...
      // Обновляем данные на графике в реальном времени ТОЛЬКО для текущей недели
      if (window.StatsPage && window.StatsPage.currentPage === 1) {
        // Проверяем, что выбрана ТЕКУЩАЯ неделя (selectedWeekOffset === 0), 
        // и только тогда обновляем график в реальном времени
        if (window.StatsPage.selectedWeekOffset === 0) {
          updateChartData();
        }
      }
    }, 1000); // Каждую секунду
  ...
  function scheduleResetAtMidnight() {
    ...
    // Вычисляем время до полуночи по московскому времени (UTC+3)
    ...
    // Создаем дату для московской полуночи
    const moscowMidnight = new Date(now);
    moscowMidnight.setHours(0, 0, 0, 0);
    moscowMidnight.setDate(moscowMidnight.getDate() + 1);
    moscowMidnight.setTime(moscowMidnight.getTime() - differenceMinutes * 60000);
    
    const timeToMidnight = moscowMidnight - now;
    ...
    resetTimeout = setTimeout(function() {
      ...
      // Проверяем и обновляем статус задач для расчета серий дней
      checkTasksCompletionForStreak();
  ...
  /**
   * Проверка статуса задач для расчета серий дней
   */
  function checkTasksCompletionForStreak() {
    // Проверяем наличие функции для обновления статистики задач
    if (window.StatsPage && typeof window.StatsPage.loadAndUpdateTasksStats === 'function') {
      window.StatsPage.loadAndUpdateTasksStats();
    }
  }
  ...
  function getWeeklyActivity(startDate, endDate) {
    const activityData = [0, 0, 0, 0, 0, 0, 0]; // Инициализируем массив нулями для всех 7 дней
    const currentDate = new Date(startDate);
    const dailyActivityData = JSON.parse(localStorage.getItem('dailyActivityData')) || {};
    
    // Перебираем дни недели
    for (let i = 0; i < 7; i++) {
      const dateKey = formatDateKey(currentDate);
      
      // Если есть данные о активности для этого дня, используем их
      if (dailyActivityData[dateKey] !== undefined) {
        // Конвертируем из секунд в минуты и округляем до целого числа
        activityData[i] = Math.floor(dailyActivityData[dateKey] / 60);
      }
      // Иначе оставляем 0, который был инициализирован по умолчанию
