...
function initializeGoalsPage() {
    ...
    // Инициализация модальных окон для просмотра целей - временно отключено
    // initializeGoalModal();
    ...
    function addNewGoal() {
        ...
        const newGoal = {
            ...
            progress: 0, // По умолчанию прогресс 0%
    ...
    function createGoalElement(goal) {
        ...
        // Убираем обработчик для просмотра цели в модальном окне
        // Оставляем элемент без дополнительного действия при клике
    ...
    // Функция для показа модального окна с деталями цели (ВРЕМЕННО ОТКЛЮЧЕНО)
    /* 
    function showGoalModal(goal) {
        ...
        modalContent.textContent = goal.content;
    ...
    */
    ...
    // Инициализация модального окна цели (ВРЕМЕННО ОТКЛЮЧЕНО)
    /*
    ...
    */
    ...
    function toggleGoalStatus(goalId, isCompleted) {
        // Находим цель по ID
        const goalIndex = goals.findIndex(goal => goal.id === goalId);
        
        if (goalIndex !== -1) {
            // Устанавливаем прогресс 100% если цель выполнена, или 0% если не выполнена
            goals[goalIndex].progress = isCompleted ? 100 : 0;
...
function loadGoals() {
    ...
    if (goalsData) {
        try {
            ...
            // Убедимся, что у всех целей есть поле progress
            goals.forEach(goal => {
