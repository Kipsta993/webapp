...
document.addEventListener('DOMContentLoaded', function() {
  ...
  let lastDeletedDate = null; // Для хранения последней удаленной даты
  ...
  if (goalTitle) {
    ...
    const parentElement = goalTitle.parentElement;
    const originalId = goalTitle.id;
    const originalClasses = goalTitle.className;
    const originalPlaceholder = goalTitle.placeholder;
    const originalRequired = goalTitle.required;
    const originalAutocomplete = goalTitle.autocomplete;
    
    // Удаляем проблемное поле
    goalTitle.remove();
    
    // Создаем новое поле с теми же атрибутами
    const newGoalTitle = document.createElement('input');
    newGoalTitle.type = 'text';
    newGoalTitle.id = originalId;
    newGoalTitle.className = originalClasses;
    newGoalTitle.placeholder = originalPlaceholder;
    newGoalTitle.required = originalRequired;
    newGoalTitle.autocomplete = originalAutocomplete;
    
    // Добавляем дополнительные стили и атрибуты для гарантии работы
    newGoalTitle.style.zIndex = '100';
    newGoalTitle.style.position = 'relative';
    newGoalTitle.style.pointerEvents = 'auto';
    
    // Добавляем новое поле в DOM
    parentElement.prepend(newGoalTitle);
  ...
  function initCalendar() {
    ...
    // Добавляем обработчики свайпов для даты
    addDateSwipeHandlers();
  }
  
  // Функция для добавления обработчиков свайпа для элемента с датой
  function addDateSwipeHandlers() {
    if (!dateDisplayButton) return;
    
    let startX = 0;
    let currentX = 0;
    
    // Обработчики только для сенсорных устройств
    dateDisplayButton.addEventListener('touchstart', function(e) {
      startX = e.touches[0].clientX;
    }, { passive: true });
    
    dateDisplayButton.addEventListener('touchmove', function(e) {
      currentX = e.touches[0].clientX;
      const diffX = currentX - startX;
      
      // Ограничиваем смещение до 30px
      const translateX = Math.min(Math.max(diffX, -30), 30);
      dateDisplayButton.style.transform = `translateX(${translateX}px)`;
    }, { passive: true });
    
    dateDisplayButton.addEventListener('touchend', function(e) {
      const diffX = currentX - startX;
      
      if (diffX > 20) {
        // Свайп вправо - удаляем дату (порог 20px)
        if (selectedDate) {
          // Сохраняем текущую дату перед очисткой
          lastDeletedDate = {
            date: selectedDate,
            formattedDate: selectedDateText.textContent,
            value: goalDeadline.value
          };
        ...
        // Свайп влево - восстанавливаем последнюю удаленную дату
        if (lastDeletedDate) {
          selectedDate = lastDeletedDate.date;
          selectedDateText.textContent = lastDeletedDate.formattedDate;
          goalDeadline.value = lastDeletedDate.value;
      ...
      dateDisplayButton.style.transform = 'translateX(0)';
    });
  }
