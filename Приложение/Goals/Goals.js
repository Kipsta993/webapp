document.addEventListener('DOMContentLoaded', function() {
    ...
    const deadlineToggle = document.getElementById('goal-deadline-toggle');
    ...
    function setupEventListeners() {
        addGoalBtn.addEventListener('click', function() {
            ...
            // Сброс формы
            goalForm.reset();
            // По умолчанию включаем дедлайн
            deadlineToggle.checked = true;
            deadlineContainer.style.display = 'block';
            deadlineStatus.textContent = 'Установить срок';
        ...
        // Обработчик для переключателя дедлайна
        deadlineToggle.addEventListener('change', function() {
            if (this.checked) {
                ...
                deadlineStatus.textContent = 'Бессрочно';
            }
        });
        ...
        goalForm.addEventListener('submit', function(event) {
            ...
            const hasDeadline = deadlineToggle.checked;
            ...
            if (hasDeadline && !deadline) {
                alert('Пожалуйста, выберите дату выполнения или отключите срок');
                return;
            }
        ...
        editGoalBtn.addEventListener('click', function() {
            if (currentGoal) {
                ...
                deadlineToggle.checked = currentGoal.hasDeadline;
    ...
    function createGoalElement(goal) {
        ...
        const formattedDate = formatDate(new Date(goal.createdAt));
        ...
        if (goal.hasDeadline) {
            ...
            deadlineHtml = `<div class="goal-deadline ${deadlineClass}">${deadlineText}</div>`;
        } else {
            deadlineHtml = '<div class="goal-deadline">Бессрочно</div>';
        }
        
        // Создаем превью описания
        ...
        goalElement.innerHTML = `
            ...
            ${deadlineHtml}
            <div class="goal-preview">${descriptionPreview}</div>
        `;
    ...
    function viewGoal(goal) {
        ...
        document.getElementById('view-goal-date').textContent = formatDate(new Date(goal.createdAt));
        document.getElementById('view-goal-description').textContent = goal.description;
        ...
        if (goal.hasDeadline) {
            ...
            deadlineElement.textContent = 'Бессрочно';
        }
