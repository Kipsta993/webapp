...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function initializeItemSizes() {
    const tasksCard = document.querySelector('#weekly-page .card.tasks-card');
    if (tasksCard && weekTasksContainer) {
      // Установка ширины контейнера week-tasks равной ширине карточки
      const cardWidth = tasksCard.offsetWidth;
      const taskItems = document.querySelectorAll('#weekly-page .task-item');
      
      // Применение размеров к существующим элементам
      taskItems.forEach(item => {
        // Вычисляем ширину элемента (на 10px меньше ширины карточки)
        item.style.width = (cardWidth - 10) + 'px';
      });
      
      console.log('Размеры элементов task-item для Weekly инициализированы. Ширина карточки: ' + cardWidth + 'px');
    }
  }
