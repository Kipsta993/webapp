...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function initializeItemSizes() {
    ...
    if (tasksCard && weekTasksContainer) {
      ...
      console.log('Weekly tasks container initialized, card width =', cardWidth);
  ...
  // Функция для сброса всех выполненных заданий
  function resetAllTasks() {
    // Получаем задания из localStorage
    const tasks = JSON.parse(localStorage.getItem('weekly-tasks')) || [];
    
    // Устанавливаем статус completed в false для всех заданий
    tasks.forEach(task => {
      task.completed = false;
    });
    
    // Сохраняем обновленные задания в localStorage
    localStorage.setItem('weekly-tasks', JSON.stringify(tasks));
    
    // Обновляем локальный массив и отображение
    allTasks = tasks;
    displayTasksForCurrentPage();
    
    console.log('Еженедельные задания сброшены');
  }
  
  // Экспортируем функцию для доступа из TaskResetter.js
  window.weeklyResetTasks = resetAllTasks;
