document.addEventListener('DOMContentLoaded', function() {
    try {
        ...
        // Проверяем, есть ли отредактированные задачи
        checkEditedTasks();
...
// Функция для проверки отредактированных задач
function checkEditedTasks() {
    const editedTasks = localStorage.getItem('weeklyTasksEditor');
    if (!editedTasks) return;
    
    try {
        const tasks = JSON.parse(editedTasks);
        ...
        // Получаем только названия задач
        const taskNames = [];
        for (const key in tasks) {
            if (key.includes('_label')) {
                taskNames.push(tasks[key]);
            }
        }
        ...
        // Удаляем сохраненные отредактированные задачи
        localStorage.removeItem('weeklyTasksEditor');
    } catch (error) {
        console.error('Ошибка при загрузке отредактированных задач:', error);
    }
}
