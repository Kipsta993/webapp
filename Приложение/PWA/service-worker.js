...
self.addEventListener('push', function(event) {
  ...
  const showNotification = self.registration.showNotification(
    notificationData.title, 
    {
      ...
      vibrate: notificationData.vibrate || [200, 100, 200, 100, 200], // Усиленная вибрация для мобильных устройств
      timestamp: notificationData.timestamp,
      requireInteraction: notificationData.requireInteraction,
      sound: 'default',
      renotify: notificationData.renotify || true // Всегда уведомлять пользователя
    }
  );
...
self.addEventListener('notificationclick', function(event) {
  ...
  // Добавляем дополнительную вибрацию при клике для обратной связи на мобильных устройствах
...
self.addEventListener('notificationclose', function(event) {
  ...
  // Здесь можно добавить аналитику закрытий уведомлений
});
