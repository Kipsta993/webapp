...
function loadTaskStates() {
    try {
        // Получаем сохраненные состояния чекбоксов
        const savedStates = JSON.parse(localStorage.getItem('dailyTaskStates')) || {};
        
        // Обновляем список задач из localStorage
        updateTaskList();
...
function updateTaskList(tasks) {
    ...
    // Если задачи не переданы, загружаем их из localStorage
    const dailyTasks = tasks || JSON.parse(localStorage.getItem('dailyTasks')) || [
        "Воркаут в \"Lumosity\"",
        "Воркаут в \"Peak\"",
        "Перевести 1 главу манги"
    ];
    
    // Добавляем задачи из массива
...
function saveTaskStates() {
    try {
        ...
        localStorage.setItem('dailyTaskStates', JSON.stringify(states));
    } catch (error) {
...
function updateNextTask() {
    try {
        ...
        schedule.forEach(task => {
            ...
            // Вычисляем разницу в минутах между текущим временем и временем начала задания
            const currentTimeInMinutes = currentHours * 60 + currentMinutes;
            const startTimeInMinutes = startHours * 60 + startMinutes;
            
            // Если задание еще не началось и оно ближе, чем текущее ближайшее
            if (startTimeInMinutes > currentTimeInMinutes && startTimeInMinutes - currentTimeInMinutes < minTimeDiff) {
                minTimeDiff = startTimeInMinutes - currentTimeInMinutes;
                nextTask = task;
            }
        });
        ...
        if (taskCard) {
            ...
            if (nextTask) {
                ...
                taskTitle.textContent = nextTask.task;
            } else {
        ...
        console.error('Ошибка при обновлении следующего задания:', error);
    }
}
...
function resetDailyTasks() {
    try {
        ...
        checkboxes.forEach(checkbox => {
            ...
            // Удаляем класс completed-task у метки
            const label = document.querySelector(`label[for="${checkbox.id}"]`);
            if (label) {
                label.classList.remove('completed-task');
            }
            
            // Сбрасываем стиль у родительского элемента
            const taskItem = checkbox.closest('.task-item');
            if (taskItem) {
