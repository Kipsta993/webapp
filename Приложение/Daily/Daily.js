        ...
        <div class="schedule-modal-content">
            ...
            <div class="schedule-modal-body">
                ...
                <div class="schedule-items-container">
                    <div class="schedule-items" id="schedule-items-${day}">
...
function generateScheduleItems(day) {
    const schedule = weeklySchedule[day.toLowerCase()];
...
function addScheduleButton() {
    ...
    // Добавляем кнопку в правую часть заголовка
    headerRight.appendChild(scheduleButton);
...
function showCurrentTaskInHeader() {
    ...
    if (currentTask) {
        ...
        const [endHours, endMinutes] = currentTask.endTime.split(':').map(Number);
        const endTimeInMinutes = endHours * 60 + endMinutes;
        ...
        const [startHours, startMinutes] = nextTask.startTime.split(':').map(Number);
        const startTimeInMinutes = startHours * 60 + startMinutes;
        ...
        nextTaskElement.innerHTML = `
            <div class="next-task-title">
                ...
                <span>Следующая задача: ${nextTask.name}</span>
            </div>
    ...
    for (let i = 0; i < daySchedule.length; i++) {
        ...
        if (timeUntilTask > 0 && timeUntilTask <= 5) {
            ...
            const notification = new Notification("Скоро начнется задача", {
                body: `${task.name} начнется через ${timeUntilTask} мин`,
                icon: "/favicon.ico"
            });
