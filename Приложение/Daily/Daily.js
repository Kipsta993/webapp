document.addEventListener('DOMContentLoaded', function() {
    ...
    // Объекты для хранения таймеров анимаций
    ...
    checkboxes.forEach(checkbox => {
        ...
        checkbox.addEventListener('change', function() {
            ...
            if (this.checked) {
                ...
                if (taskItem) {
                    // Добавляем класс для анимации пульсации с плавностью
                    const taskId = taskItem.id || 'task-' + Math.random();
                    taskItem.id = taskId;
                    
                    // Очищаем предыдущий таймер, если он существует
                    if (animationTimers.taskItems[taskId]) {
                        clearTimeout(animationTimers.taskItems[taskId]);
                    }
                    ...
                    animationTimers.taskItems[taskId] = setTimeout(() => {
                        taskItem.classList.remove('completed');
                        delete animationTimers.taskItems[taskId];
                    }, 500);
                ...
                // Анимация увеличения чекбокса с плавностью
                const checkboxId = this.id || 'checkbox-' + Math.random();
                this.id = checkboxId;
                
                // Очищаем предыдущий таймер, если он существует
                if (animationTimers.checkboxes[checkboxId]) {
                    clearTimeout(animationTimers.checkboxes[checkboxId]);
                }
                ...
                // Устанавливаем новый таймер
                animationTimers.checkboxes[checkboxId] = setTimeout(() => {
                    this.classList.remove('checkbox-scale');
                    delete animationTimers.checkboxes[checkboxId];
                }, 300);
    ...
    taskItems.forEach(taskItem => {
        taskItem.addEventListener('click', function(event) {
            ...
            if (!event.target.matches('input[type="checkbox"]') && 
                ...
                // Проверяем, был ли клик на иконке или её дочерних элементах
                const isIconClick = event.target.matches('.task-icon') || 
                                   event.target.matches('.task-icon img') || 
                                   event.target.closest('.task-icon');
                
                // Если клик был на иконке, добавляем задержку перед обработкой
                // чтобы избежать проблем с быстрыми множественными кликами
                if (isIconClick) {
                    // Используем debounce для предотвращения множественных быстрых кликов
                    if (this.iconClickTimeout) {
                        clearTimeout(this.iconClickTimeout);
                    }
                    
                    this.iconClickTimeout = setTimeout(() => {
                        processTaskClick(this);
                    }, 50); // Небольшая задержка для предотвращения множественных кликов
                } else {
                    // Для остальных областей обрабатываем клик сразу
                    processTaskClick(this);
                }
            }
        });
    ...
    function processTaskClick(taskItem) {
        ...
        // Если задание уже выполнено, проигрываем только анимацию
            ...
            // Анимация пульсации с плавностью
            const taskId = taskItem.id || 'task-' + Math.random();
            taskItem.id = taskId;
            ...
            // Анимация увеличения чекбокса с плавностью
            const checkboxId = checkbox.id || 'checkbox-' + Math.random();
