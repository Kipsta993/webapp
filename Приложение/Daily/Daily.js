    ...
    for (const taskId in deletedTasks) {
        if (deletedTasks.hasOwnProperty(taskId)) {
            ...
            if (taskItem && taskItem.closest('.task-item')) {
                ...
                console.log(`Удаленная задача ${taskId} скрыта при загрузке страницы Daily`);
            }
        }
    }
    ...
    if (savedTasks) {
        ...
        checkboxes.forEach(checkbox => {
            if (tasks[checkbox.id] !== undefined) {
                ...
                // Если задача отмечена, применяем соответствующие стили
        ...
        const savedTasksOld = localStorage.getItem('dailyTasksCompleted');
        if (savedTasksOld) {
            const tasksOld = JSON.parse(savedTasksOld);
            const checkboxes = document.querySelectorAll('input[type="checkbox"]');
            
            // Создаем новый формат задач
            const newTasks = {};
            
            checkboxes.forEach(checkbox => {
                if (tasksOld[checkbox.id]) {
                    checkbox.checked = true;
                    newTasks[checkbox.id] = true;
