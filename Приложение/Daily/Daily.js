...
function loadTaskStates() {
    ...
    if (savedTasks) {
        ...
        // Проверяем, есть ли сохраненные тексты заданий
        const hasTaskTexts = Object.keys(tasks).some(key => key.endsWith('_text'));
        
        if (hasTaskTexts) {
            // Новый формат с текстами заданий
            // Сначала очищаем список заданий
            const taskList = document.querySelector('.all-day-tasks .task-list');
            if (taskList) {
                taskList.innerHTML = '';
                
                // Создаем задания из сохраненных данных
                const taskIds = new Set();
                Object.keys(tasks).forEach(key => {
                    if (key.endsWith('_text')) {
                        const taskId = key.replace('_text', '');
                        if (!taskIds.has(taskId)) {
                            ...
                            checkbox.id = taskId;
                            checkbox.checked = tasks[taskId] || false;
                            if (tasks[taskId + '_disabled']) {
                                checkbox.disabled = true;
                            }
                            
                            const label = document.createElement('label');
                            label.htmlFor = taskId;
                            label.textContent = tasks[key];
                            if (tasks[taskId + '_disabled']) {
                                label.classList.add('completed-task');
                            }
                ...
                // Инициализируем чекбоксы и обработчики
                initializeCheckboxes();
                initializeTaskItems();
            }
        } else {
            // Старый формат без текстов заданий
        checkboxes.forEach(checkbox => {
...
function saveTaskStates() {
    ...
    checkboxes.forEach(checkbox => {
        ...
        const label = document.querySelector(`label[for="${checkbox.id}"]`);
        if (label) {
            tasks[checkbox.id + '_text'] = label.textContent;
        }
    });
...
function updateNextTask() {
    ...
    let weeklySchedule = window.weeklySchedule || {}; // Используем глобальное расписание как запасной вариант
    ...
    if (!daySchedule || daySchedule.length === 0) {
        ...
        updateTaskCard("Нет активных заданий", "");
        return;
    }
    ...
    // Находим ближайшее следующее задание
    for (const task of daySchedule) {
        ...
        // Если задание еще не началось и оно ближе, чем текущее найденное
        if (startTimeMinutes > currentTimeMinutes && startTimeMinutes - currentTimeMinutes < minTimeDiff) {
            nextTask = task;
            minTimeDiff = startTimeMinutes - currentTimeMinutes;
        }
    }
    
    // Если следующее задание не найдено, ищем текущее задание
    if (!nextTask) {
        for (const task of daySchedule) {
            const [startHours, startMinutes] = task.start.split(':').map(Number);
            ...
            // Проверяем, находится ли текущее время в интервале задания
            if (currentTimeMinutes >= startTimeMinutes && currentTimeMinutes < endTimeMinutes) {
                updateTaskCard(task.title, `${task.start} - ${task.end}`);
                return;
            }
        }
        
        // Если ни текущего, ни следующего задания нет, показываем первое задание следующего дня
        ...
        if (nextDaySchedule && nextDaySchedule.length > 0) {
            ...
            nextDaySchedule.sort((a, b) => {
                const timeA = a.start.split(':').map(Number);
                const timeB = b.start.split(':').map(Number);
                return (timeA[0] * 60 + timeA[1]) - (timeB[0] * 60 + timeB[1]);
            });
            
            const firstTask = nextDaySchedule[0];
            updateTaskCard(`Завтра: ${firstTask.title}`, `${firstTask.start} - ${firstTask.end}`);
        } else {
            updateTaskCard("Нет активных заданий", "");
        }
    } else {
        // Если найдено следующее задание, отображаем его
        updateTaskCard(nextTask.title, `${nextTask.start} - ${nextTask.end}`);
    }
}

// Вспомогательная функция для обновления карточки задания
function updateTaskCard(title, time) {
...
function showScheduleModal(selectedDay = null, direction = null) {
    // Проверяем, доступна ли функция openScheduleModal из нового редактора
    if (typeof openScheduleModal === 'function') {
        // Используем новый редактор расписания
        openScheduleModal(selectedDay);
        return;
    }
    
    // Если новый редактор недоступен, используем старую реализацию
    ...
    if (modal.style.display === 'flex') {
        ...
        if (daySchedule && daySchedule.length > 0) {
            ...
            newSchedule.innerHTML = `<div class="no-schedule">Нет расписания на этот день</div>`;
        }
        
        // Добавляем новое расписание и удаляем старое после анимации
        ...
        // Если модальное окно закрыто, открываем его с новым содержимым
    ...
    prevDayButton.addEventListener('click', () => {
            ...
            showScheduleModal(days[prevDayIndex], 'right');
    });
    
    nextDayButton.addEventListener('click', () => {
            const nextDayIndex = (currentDayIndex + 1) % days.length;
            showScheduleModal(days[nextDayIndex], 'left');
        });
    ...
    function handleSwipe() {
        const scheduleElement = modal.querySelector('.day-schedule');
        let startX, startY, endX, endY;
        let minSwipeDistance = 50;
        
        scheduleElement.addEventListener('touchstart', function(e) {
            startX = e.touches[0].clientX;
            startY = e.touches[0].clientY;
        });
        
        scheduleElement.addEventListener('touchend', function(e) {
