        ...
        taskItem.addEventListener('click', function(event) {
            ...
            if (!event.target.matches('input[type="checkbox"]') && 
                ...
                if (checkbox && !checkbox.checked && !checkbox.disabled) {
                    ...
                    // Вызываем событие change вручную, чтобы сработали все обработчики
                    const event = new Event('change');
                    checkbox.dispatchEvent(event);
                }
                // Добавляем анимацию даже для выполненных заданий при клике на область картинки
                else if (checkbox && checkbox.checked && checkbox.disabled && 
                        (event.target.matches('.task-icon') || event.target.matches('.task-icon img'))) {
