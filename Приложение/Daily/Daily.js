...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function updateNextTask() {
    ...
    // Находим ближайшее будущее задание
    let nextItem = null;
    ...
    if (currentItem) {
      taskContent.innerHTML = `
        <div class="task-time-small">Сейчас: ${formatTimeRange(currentItem.startTime, currentItem.endTime)}</div>
        <div class="task-title">${currentItem.title}</div>
      `;
      return;
    }
    
    // Если есть следующее задание, показываем его
    if (nextItem) {
      taskContent.innerHTML = `
        <div class="task-time-small">Следующее: ${formatTimeRange(nextItem.startTime, nextItem.endTime)}</div>
        <div class="task-title">${nextItem.title}</div>
      `;
      return;
    }
    
    // Если нет ни текущего, ни следующего задания, показываем сообщение
    taskContent.innerHTML = `
      <div class="task-time-small">Нет запланированных заданий</div>
      <div class="task-title">На сегодня больше ничего не запланировано</div>
    `;
  }
