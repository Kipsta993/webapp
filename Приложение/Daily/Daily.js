document.addEventListener('DOMContentLoaded', function() {
    ...
    // Добавляем обработчики для меток, чтобы они корректно отмечали чекбоксы
    ...
    labels.forEach(label => {
        label.addEventListener('click', function() {
            ...
            if (checkbox && !checkbox.disabled && !checkbox.checked) {
                // Чекбокс будет отмечен автоматически благодаря атрибуту 'for'
                // Но мы можем добавить дополнительные эффекты здесь, если нужно
            }
        });
    });
