document.addEventListener('DOMContentLoaded', function() {
    ...
    // Проверяем, были ли обновлены задачи в настройках
    checkTasksUpdated();
});
...
function updateNextTask() {
    ...
    // Получаем задачи расписания из localStorage или используем дефолтные
    const scheduleTasks = JSON.parse(localStorage.getItem('scheduleTasks')) || window.scheduleTasks || [];
    ...
    scheduleTasks.forEach(task => {
        // Парсим время начала задачи
        const [startHour, startMinute] = task.startTime.split(':').map(Number);
        
        // Вычисляем разницу во времени в минутах
        let timeDiff = (startHour - currentHour) * 60 + (startMinute - currentMinute);
        
        // Если задача уже началась, но еще не закончилась, считаем ее текущей
        const [endHour, endMinute] = task.endTime.split(':').map(Number);
        const endTimeDiff = (endHour - currentHour) * 60 + (endMinute - currentMinute);
        
        if (timeDiff <= 0 && endTimeDiff > 0) {
            timeDiff = 0; // Текущая задача
        }
        
        // Если задача еще не началась или текущая, и она ближе всех по времени
    ...
    if (taskCard) {
        ...
        if (nextTask) {
            taskTime.textContent = `${nextTask.startTime} - ${nextTask.endTime}`;
            taskTitle.textContent = nextTask.text;
            
            // Если задача текущая, выделяем ее
            if (minTimeDiff === 0) {
            ...
            taskTitle.textContent = 'Нет активных заданий';
            taskCard.classList.remove('active-task');
        }
    }
}
...
function resetDailyTasks() {
    ...
    const checkboxes = document.querySelectorAll('.all-day-tasks input[type="checkbox"]');
    ...
    checkboxes.forEach(checkbox => {
        ...
        // Сбрасываем стили для родительского элемента
        const taskItem = checkbox.closest('.task-item');
        if (taskItem) {
            taskItem.style.borderLeftColor = '';
        }
        
        // Сбрасываем стили для метки
        const label = document.querySelector(`label[for="${checkbox.id}"]`);
        if (label) {
    ...
    const taskStates = {};
    checkboxes.forEach(checkbox => {
        taskStates[checkbox.id] = {
            checked: false,
            disabled: false
        };
    });
    localStorage.setItem('dailyTaskStates', JSON.stringify(taskStates));
    
    // Сохраняем текущие задачи, если они еще не сохранены
    if (!localStorage.getItem('dailyTasks')) {
        const dailyTasks = [];
        checkboxes.forEach(checkbox => {
            const label = document.querySelector(`label[for="${checkbox.id}"]`);
            if (label) {
                dailyTasks.push(label.textContent);
            }
        });
...
function loadTasksFromStorage() {
    ...
    if (scheduleTasks && scheduleTasks.length > 0) {
        // Обновляем глобальный массив задач расписания
        window.scheduleTasks = scheduleTasks;
    }
}

// Проверка обновления задач из настроек
function checkTasksUpdated() {
    const tasksUpdated = localStorage.getItem('tasksUpdated');
    if (tasksUpdated === 'true') {
        // Перезагружаем задачи
        loadTasksFromStorage();
        
        // Инициализируем чекбоксы заново
        initializeCheckboxes();
        
        // Инициализируем обработчики кликов заново
        initializeTaskItems();
        
        // Обновляем следующую задачу
        updateNextTask();
        
        // Сбрасываем флаг обновления
        localStorage.setItem('tasksUpdated', 'false');
    }
}
