...
function initializeNavigation() {
    ...
    const currentPage = 'diary'; // Текущая страница - дневник
...
function createEntryElement(entry) {
    ...
    // Добавляем метку "редактировано", если заметка была отредактирована
    if (entry.edited) {
        const editedMark = document.createElement('span');
        editedMark.className = 'entry-edited-mark';
        editedMark.textContent = '(Редактировано)';
        entryTitle.appendChild(editedMark);
    }
...
function viewEntry(entry) {
    ...
    // Заполняем модальное окно данными заметки
    ...
    // Скрываем метку "редактировано" в модальном окне, так как она теперь отображается в главном экране
    document.getElementById('edited-mark').style.display = 'none';
    
    // Очищаем контейнер изображений
    const imageContainer = document.getElementById('view-entry-image-container');
    imageContainer.innerHTML = '';
    
    // Если у заметки есть изображения, создаем слайдер
...
function updateEntryInStorage(updatedEntry) {
    ...
    if (index !== -1) {
        ...
        // Увеличиваем счетчик отредактированных записей
        incrementEditedEntriesCount();
...
// Функция для увеличения счетчика отредактированных записей
function incrementEditedEntriesCount() {
    let editedCount = parseInt(localStorage.getItem('diaryEditedCount') || '0');
    editedCount++;
    localStorage.setItem('diaryEditedCount', editedCount.toString());
}
...
function deleteEntryFromStorage(entryId) {
    ...
    incrementDeletedEntriesCount();
    
    // Обновление отображения заметок
    loadEntries();
}

// Функция для увеличения счетчика удаленных записей
function incrementDeletedEntriesCount() {
    let deletedCount = parseInt(localStorage.getItem('diaryDeletedCount') || '0');
    deletedCount++;
    localStorage.setItem('diaryDeletedCount', deletedCount.toString());
}
