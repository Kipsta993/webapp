...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (!entryForm || !entriesList || !entriesContainer || !modal || !fullscreenPhoto) return;
        ...
        if (entry.photos && entry.photos.length > 0) {
          photosIndicator = `
            <div class="entry-photos-indicator">
              <i class="fas fa-camera"></i> ${entry.photos.length} фото
            </div>
          `;
        }
