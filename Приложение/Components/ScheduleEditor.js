...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function createCustomTimeSelector(inputElement) {
    ...
    // Добавляем id и name для поля ввода часов
    const inputId = inputElement.id || '';
    hoursInput.id = inputId ? `${inputId}-hours` : `time-hours-${Math.random().toString(36).substr(2, 9)}`;
    hoursInput.name = inputId ? `${inputId}-hours` : `time-hours-${Math.random().toString(36).substr(2, 9)}`;
    ...
    // Добавляем id и name для поля ввода минут
    minutesInput.id = inputId ? `${inputId}-minutes` : `time-minutes-${Math.random().toString(36).substr(2, 9)}`;
    minutesInput.name = inputId ? `${inputId}-minutes` : `time-minutes-${Math.random().toString(36).substr(2, 9)}`;
