...
function checkScheduleNotifications() {
  ...
  if (!notificationsEnabled) {
    // Для уменьшения объема логов, убираем это сообщение, так как оно будет слишком часто повторяться
    return;
  }
  ...
  // Получаем текущее время с секундной точностью
  const now = new Date();
  const currentHours = now.getHours();
  const currentMinutes = now.getMinutes();
  ...
  // Для логирования секунд, создаем более детальный формат времени
  const currentDetailedTime = `${currentTimeString}:${currentSeconds.toString().padStart(2, '0')}`;
  
  // Логируем текущее время только раз в минуту (когда секунды равны 0), чтобы не переполнять консоль
  if (currentSeconds === 0) {
    console.log(`⏰ Проверка уведомлений: текущее время ${currentDetailedTime}`);
  }
  ...
  scheduleItems.forEach(item => {
    ...
    if (isStartTimeMatched) {
      ...
      console.log(`🔔 Отправка уведомления для задания: ${item.title} в ${currentDetailedTime}`);
