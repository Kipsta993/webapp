...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function checkNewItemForNotification(item) {
    ...
    // Если время начала задания совпадает с текущим временем или находится в пределах 1 минуты
    if (item.startTime === currentTimeString) {
      ...
      if (Notification.permission === 'granted') {
        sendNotificationForItem(item, now);
      }
    }
  }
...
// Выделенная функция для отправки уведомления для конкретного задания
function sendNotificationForItem(item, now) {
  // Создаем уведомление через Service Worker
  if (navigator.serviceWorker && navigator.serviceWorker.controller) {
    navigator.serviceWorker.ready.then(registration => {
      console.log('Используем Service Worker для отправки уведомления');
      registration.showNotification('Начало задания', {
        ...
        tag: `task-${item.id}`, // Уникальный тег для каждого задания
        renotify: true // Уведомлять пользователя каждый раз, даже если тег тот же
      }).then(() => {
        // Сохраняем время отправки уведомления только при успехе
        localStorage.setItem(`notification_${item.id}`, now.toISOString());
        console.log(`Уведомление через Service Worker успешно отправлено для задания "${item.title}"`);
      }).catch(error => {
        console.error('Ошибка при отправке уведомления через Service Worker:', error);
        // Если произошла ошибка, используем стандартный API
        sendStandardNotification(item, now);
      });
    }).catch(error => {
...
// Выделенная функция для отправки стандартного уведомления
function sendStandardNotification(item, now) {
  try {
