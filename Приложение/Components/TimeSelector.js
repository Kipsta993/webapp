...
document.addEventListener('DOMContentLoaded', function() {
  ...
  timeInputs.forEach(input => {
    // Пропускаем поля, которые уже обрабатываются в ScheduleEditor.js
    if (input.id === 'schedule-start-time' || input.id === 'schedule-end-time') {
      console.log(`Пропускаем инициализацию TimeSelector для ${input.id}, т.к. он обрабатывается в ScheduleEditor.js`);
      return;
    }
    
    // Проверяем, не находится ли поле уже внутри .custom-time-selector
    if (input.parentNode.classList.contains('custom-time-selector')) {
      console.log(`Пропускаем инициализацию TimeSelector для ${input.id}, т.к. он уже обрабатывается другим скриптом`);
      return;
    }
