...
function loadTasksIndependently() {
    ...
    const isRootPage = currentPath.endsWith('index.html') || currentPath.endsWith('/');
    const isInSubfolder = currentPath.includes('Приложение/');
    
    // Формируем правильные пути к файлам в зависимости от текущего местоположения
    let dailyPath = isRootPage ? 'index.html' : (isInSubfolder ? '../../index.html' : 'index.html');
    let weeklyPath = isRootPage ? 'Приложение/Weekly/Weekly.html' : (isInSubfolder ? '../Weekly/Weekly.html' : 'Приложение/Weekly/Weekly.html');
        ...
        fetch(dailyPath)
            ...
            .then(html => {
                ...
                checkboxes.forEach(checkbox => {
                    ...
                    // Пропускаем удаленные задачи
                    if (deletedTasks[taskId]) {
                        console.log(`Пропускаем удаленную задачу: ${taskId}`);
                        return;
                    }
