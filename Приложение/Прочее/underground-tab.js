...
// Функция для загрузки задач из Weekly.html
function loadWeeklyTasksFromPage() {
    return new Promise((resolve, reject) => {
        // Проверяем, есть ли уже сохраненные задачи в localStorage
        const savedWeeklyTasks = localStorage.getItem('weeklyTasks');
        if (savedWeeklyTasks) {
            try {
                const weeklyTasks = JSON.parse(savedWeeklyTasks);
                // Фильтруем только реальные задачи (не служебные ключи)
                const filteredTasks = {};
                for (const key in weeklyTasks) {
                    if (weeklyTasks.hasOwnProperty(key) && !key.includes('_disabled')) {
                        filteredTasks[key] = weeklyTasks[key];
                    }
                }
                ...
                console.error('Ошибка при парсинге сохраненных еженедельных задач:', e);
            }
        }
        
        // Если нет сохраненных задач или они пусты, загружаем из файла
        fetch('Приложение/Weekly/Weekly.html')
            ...
            .then(html => {
                ...
                const weeklyTasks = {};
                const checkboxes = doc.querySelectorAll('.task-item input[type="checkbox"]');
                
                checkboxes.forEach(checkbox => {
                    weeklyTasks[checkbox.id] = false; // По умолчанию задачи не выполнены
                });
                
                resolve(weeklyTasks);
            })
            .catch(error => {
                console.error('Ошибка при загрузке Weekly.html:', error);
                reject(error);
            });
    });
}
...
function loadTasksIntoManager() {
    ...
    if (dailyTasksList) {
        ...
        // Получаем все существующие задачи на странице
        const existingDailyTasks = {};
        document.querySelectorAll('.all-day-tasks .task-item input[type="checkbox"]').forEach(checkbox => {
            existingDailyTasks[checkbox.id] = true;
        });
        
        // Фильтруем только существующие задачи
        const validTasks = Object.keys(dailyTasks).filter(taskId => existingDailyTasks[taskId]);
    ...
    if (weeklyTasksList) {
        ...
        const weeklyTasksContainer = document.querySelector('.weekly-tasks .task-list');
        
        if (!weeklyTasksContainer) {
            // Если страница Weekly не открыта, загружаем задачи из Weekly.html
            loadWeeklyTasksFromPage()
                .then(weeklyTasks => {
                    ...
                    if (Object.keys(weeklyTasks).length === 0) {
                        ...
                        // Получаем текст задач из Weekly.html
                        fetch('Приложение/Weekly/Weekly.html')
                            .then(response => response.text())
                            .then(html => {
                                const parser = new DOMParser();
                                ...
                                for (const taskId in weeklyTasks) {
                                    if (weeklyTasks.hasOwnProperty(taskId)) {
                                        // Получаем текст задачи из документа
                                        ...
                                        // Создаем элемент задачи с текстом
                                        const taskElement = document.createElement('div');
                                        taskElement.className = 'task-item-settings';
                                        
                                        taskElement.innerHTML = `
                                            <div class="task-info">
                                                <div class="task-checkbox ${weeklyTasks[taskId] ? 'checked' : ''}">
                            ...
                            .catch(error => {
                                console.error('Ошибка при получении текста задач:', error);
            ...
            // Если страница Weekly открыта, берем задачи напрямую из DOM
            weeklyTasksList.innerHTML = '';
            const weeklyCheckboxes = document.querySelectorAll('.weekly-tasks .task-item input[type="checkbox"]');
            
            if (weeklyCheckboxes.length === 0) {
                weeklyTasksList.innerHTML = '<div class="no-tasks">Нет еженедельных задач</div>';
            } else {
                weeklyCheckboxes.forEach(checkbox => {
                    const isChecked = checkbox.checked;
                    const taskElement = createTaskElement(checkbox.id, isChecked, 'weekly');
                    if (taskElement) {
                        weeklyTasksList.appendChild(taskElement);
                    }
                });
...
function createTaskElement(taskId, isChecked, taskType) {
    ...
    if (taskType === 'daily') {
        ...
        if (label) {
            ...
            // Если label не найден, задача не существует
            return null;
        }
    ...
    taskElement.innerHTML = `
        ...
        <div class="task-actions">
            <button class="edit-task-btn" data-task-id="${taskId}" data-task-type="${taskType}" data-task-text="${encodeURIComponent(taskText)}">
                <i class="fas fa-edit"></i>
            </button>
