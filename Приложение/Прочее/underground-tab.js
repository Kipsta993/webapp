...
function initSwipeDetection() {
    ...
    swipeHandle.addEventListener('touchstart', function(e) {
        ...
        isTabOpen = undergroundTab && undergroundTab.classList.contains('visible');
    });
    ...
    swipeHandle.addEventListener('touchmove', function(e) {
        ...
        const diff = currentY - startY; // Для свайпа вниз
        
        // Если вкладка закрыта и свайп вниз - открываем вкладку
        if (!isTabOpen && diff > 30) {
            e.preventDefault();
            showUndergroundTab();
            startY = 0; // Сбрасываем, чтобы предотвратить повторное срабатывание
        }
        
        // Важно: мы НЕ закрываем вкладку при продолжении свайпа вниз
        // Это позволяет пользователю продолжать свайп вниз без закрытия вкладки
    });
    ...
    // Добавляем обработчик для закрытия вкладки при свайпе снизу вверх
    const undergroundTab = document.querySelector('.underground-tab');
    if (undergroundTab) {
        let tabStartY = 0;
        
        undergroundTab.addEventListener('touchstart', function(e) {
            // Начинаем отслеживать свайп только если вкладка открыта
        ...
        undergroundTab.addEventListener('touchmove', function(e) {
            if (!tabStartY || !this.classList.contains('visible')) return;
            
            const currentY = e.touches[0].clientY;
            const diff = tabStartY - currentY; // Для свайпа вверх
            
            // Закрываем только при свайпе снизу вверх
            if (diff > 50) {
                hideUndergroundTab();
                tabStartY = 0;
            }
