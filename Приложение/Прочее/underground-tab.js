...
function createUndergroundTab() {
    ...
    taskManagerSettings.innerHTML = `
        ...
        <div class="task-content active" id="daily-tasks-content">
            <div class="task-list-container">
                ...
                <div class="task-list" id="daily-tasks-list">
                    <div class="loading-tasks">Загрузка задач...</div>
                </div>
            </div>
        </div>
        ...
        <div class="task-content" id="weekly-tasks-content">
            <div class="task-list-container">
                ...
                <div class="task-list" id="weekly-tasks-list">
                    <div class="loading-tasks">Загрузка задач...</div>
                </div>
            </div>
        </div>
    ...
    // Добавляем контент в подземную вкладку
...
function initSettingsHandlers() {
    ...
    setTimeout(() => {
        loadTasksIndependently();
        console.log('Задачи загружены при инициализации настроек');
    }, 100);
}
...
function showUndergroundTab() {
    ...
    if (undergroundTab) {
        console.log('Открываем подземную вкладку настроек');
        undergroundTab.classList.add('visible');
        
        // Обновляем активную вкладку в нижней навигации
        updateBottomNavigation();
        
        // Инициализируем обработчики событий для вкладок задач
        const taskTabs = undergroundTab.querySelectorAll('.task-tab');
        const taskContents = undergroundTab.querySelectorAll('.task-content');
        
        console.log('Найдено вкладок задач:', taskTabs.length);
        console.log('Найдено контейнеров содержимого задач:', taskContents.length);
        ...
        // Принудительно загружаем задачи при открытии настроек
        setTimeout(() => {
            // Проверяем, какая вкладка активна
            const activeTab = undergroundTab.querySelector('.task-tab.active');
            if (activeTab) {
                console.log('Активная вкладка при открытии:', activeTab.getAttribute('data-tab'));
            } else {
                console.log('Активная вкладка не найдена, устанавливаем вкладку "daily"');
                const dailyTab = undergroundTab.querySelector('.task-tab[data-tab="daily"]');
                if (dailyTab) {
                ...
                taskContents.forEach(content => {
                    content.classList.remove('active');
                    if (content.id === 'daily-tasks-content') {
                        content.classList.add('active');
                    }
                });
            }
            
            // Загружаем задачи с небольшой задержкой, чтобы DOM успел обновиться
            loadTasksIndependently();
            console.log('Задачи загружены при открытии настроек');
            
            // Инициализируем обработчики для кнопок добавления задач
            const addDailyTaskBtn = document.getElementById('add-daily-task');
            if (addDailyTaskBtn) {
                // Удаляем существующие обработчики
                const newBtn = addDailyTaskBtn.cloneNode(true);
                addDailyTaskBtn.parentNode.replaceChild(newBtn, addDailyTaskBtn);
            ...
            if (addWeeklyTaskBtn) {
                // Удаляем существующие обработчики
                const newBtn = addWeeklyTaskBtn.cloneNode(true);
                addWeeklyTaskBtn.parentNode.replaceChild(newBtn, addWeeklyTaskBtn);
...
function loadTasksIndependently() {
    ...
    console.log('Активная вкладка задач:', activeTabType);
    
    // Загружаем список удаленных задач
    const deletedTasks = JSON.parse(localStorage.getItem('deletedTasks') || '{}');
    console.log('Список удаленных задач:', deletedTasks);
    
    // Загружаем ежедневные задачи, если активна вкладка "daily"
    const dailyTasksList = document.getElementById('daily-tasks-list');
    if (dailyTasksList && (activeTabType === 'daily')) {
        console.log('Начинаем загрузку ежедневных задач');
        ...
        try {
            ...
            console.log('Загруженные данные ежедневных задач:', { dailyTasks, dailyTasksText });
            
            // Создаем массив для элементов задач
            const taskElements = [];
            
            // Получаем все ID задач из localStorage
            const taskIds = new Set();
            
            // Добавляем ID из dailyTasks
            Object.keys(dailyTasks).forEach(id => {
                if (!id.includes('_disabled') && !deletedTasks[id]) {
                    taskIds.add(id);
                }
            });
            
            // Добавляем ID из dailyTasksText
            Object.keys(dailyTasksText).forEach(id => {
                if (!deletedTasks[id]) {
                    taskIds.add(id);
                }
            });
            
            console.log('Найденные ID ежедневных задач:', Array.from(taskIds));
            
            // Если нет задач, показываем сообщение
            if (taskIds.size === 0) {
                console.log('Ежедневные задачи не найдены');
            ...
            taskIds.forEach(taskId => {
                ...
                const taskText = dailyTasksText[taskId] || 'Задача без названия';
                
                // Проверяем, выполнена ли задача
                const isChecked = dailyTasks[taskId] || false;
                
                console.log(`Создаем элемент для задачи ${taskId}: "${taskText}", выполнена: ${isChecked}`);
            ...
            console.log(`Добавляем ${taskElements.length} ежедневных задач в список`);
    ...
    if (weeklyTasksList && (activeTabType === 'weekly')) {
        console.log('Начинаем загрузку еженедельных задач');
        ...
        try {
            ...
            console.log('Загруженные данные еженедельных задач:', { weeklyTasks, weeklyTasksText });
            ...
            // Добавляем ID из weeklyTasks
            Object.keys(weeklyTasks).forEach(id => {
                if (!id.includes('_disabled') && !deletedTasks[id]) {
                    taskIds.add(id);
                }
            });
            
            // Добавляем ID из weeklyTasksText
            Object.keys(weeklyTasksText).forEach(id => {
                if (!deletedTasks[id]) {
                    taskIds.add(id);
                }
            });
            
            console.log('Найденные ID еженедельных задач:', Array.from(taskIds));
            
            // Если нет задач, показываем сообщение
            if (taskIds.size === 0) {
                console.log('Еженедельные задачи не найдены');
            ...
            taskIds.forEach(taskId => {
                // Получаем текст задачи
                const taskText = weeklyTasksText[taskId] || 'Задача без названия';
                
                // Проверяем, выполнена ли задача
                const isChecked = weeklyTasks[taskId] || false;
                const isDisabled = weeklyTasks[taskId + '_disabled'] || false;
                
                console.log(`Создаем элемент для задачи ${taskId}: "${taskText}", выполнена: ${isChecked}, отключена: ${isDisabled}`);
            ...
            console.log(`Добавляем ${taskElements.length} еженедельных задач в список`);
            taskElements.forEach(element => {
...
function deleteTask(taskId, taskType) {
    console.log(`Удаление задачи: ${taskId}, тип: ${taskType}`);
    ...
    if (taskType === 'daily') {
        ...
        if (taskItem && taskItem.closest('.task-item')) {
            ...
            console.log(`Задача ${taskId} удалена из DOM`);
        } else {
            console.log(`Элемент задачи ${taskId} не найден в DOM, удаляем только из localStorage`);
        }
        ...
        // Удаляем текст задачи из localStorage
        const dailyTasksText = JSON.parse(localStorage.getItem('dailyTasksText') || '{}');
        delete dailyTasksText[taskId];
        localStorage.setItem('dailyTasksText', JSON.stringify(dailyTasksText));
        
        // Добавляем задачу в список удаленных задач
        const deletedTasks = JSON.parse(localStorage.getItem('deletedTasks') || '{}');
        deletedTasks[taskId] = true;
        localStorage.setItem('deletedTasks', JSON.stringify(deletedTasks));
        
        console.log(`Задача ${taskId} добавлена в список удаленных задач`);
    } else if (taskType === 'weekly') {
        // Удаляем задачу из DOM, если страница открыта
        const taskItem = document.querySelector(`#${taskId}`);
        if (taskItem && taskItem.closest('.task-item')) {
    ...
    // Удаляем элемент из списка задач в менеджере
    const taskElement = document.querySelector(`.task-item-settings [data-task-id="${taskId}"]`);
    if (taskElement && taskElement.closest('.task-item-settings')) {
        taskElement.closest('.task-item-settings').remove();
        console.log(`Задача ${taskId} удалена из списка задач в менеджере`);
    }
    
    // Обновляем список задач в менеджере
    loadTasksIndependently();
    
    console.log('Обновление списка задач после удаления');
}
...
function showDeleteTaskConfirmation(taskId, taskType) {
    console.log(`Показываю подтверждение удаления для задачи: ${taskId}, тип: ${taskType}`);
    ...
    deleteBtn.addEventListener('click', function() {
        console.log(`Подтверждено удаление задачи: ${taskId}, тип: ${taskType}`);
        deleteTask(taskId, taskType);
        closeModal();
    });
}

// Также нужно модифицировать функцию initializeCheckboxes в Daily.js и Weekly.js
// Добавим код для проверки удаленных задач при загрузке страницы
document.addEventListener('DOMContentLoaded', function() {
    // Проверяем, есть ли удаленные задачи
    const deletedTasks = JSON.parse(localStorage.getItem('deletedTasks') || '{}');
    
    // Удаляем задачи из DOM, если они были удалены ранее
    for (const taskId in deletedTasks) {
        if (deletedTasks.hasOwnProperty(taskId)) {
            const taskItem = document.querySelector(`#${taskId}`);
            if (taskItem && taskItem.closest('.task-item')) {
                taskItem.closest('.task-item').remove();
                console.log(`Удаленная задача ${taskId} скрыта при загрузке страницы`);
            }
        }
    }
});