...
function createUndergroundTab() {
    ...
    // Добавляем настройки редактора заданий
    ...
    editorSettings.innerHTML = `
        ...
        <div class="setting-item">
            <div class="setting-label">
                <span>Показывать редактор</span>
                <p class="setting-description">Включить возможность редактирования заданий и расписания</p>
            </div>
            <label class="toggle">
    ...
    content.appendChild(editorSettings);
...
function initSettingsHandlers() {
    ...
    // Обработчик для переключателя редактора заданий
    const editorToggle = document.getElementById('editor-toggle');
    if (editorToggle) {
        editorToggle.addEventListener('change', function() {
            localStorage.setItem('editor_enabled', this.checked);
            toggleTaskEditor(this.checked);
        });
    }
...
// Функция для управления видимостью редактора заданий
function toggleTaskEditor(isEnabled) {
    const editListBtn = document.querySelector('.edit-list-btn');
    const addTaskBtn = document.querySelector('.add-task-btn');
    
    if (editListBtn) {
        editListBtn.style.display = isEnabled ? 'flex' : 'none';
    }
    
    if (addTaskBtn) {
        // Если редактор включен и мы в режиме редактирования, показываем кнопку добавления
        // В противном случае скрываем её
        if (isEnabled && document.querySelector('.container.editing-mode')) {
...
function loadSettings() {
    ...
    // Загружаем настройку редактора заданий
    const editorEnabled = localStorage.getItem('editor_enabled') !== 'false';
    const editorToggle = document.getElementById('editor-toggle');
    if (editorToggle) {
        editorToggle.checked = editorEnabled;
    }
    
    // Применяем настройку видимости редактора
    toggleTaskEditor(editorEnabled);
}
...
function resetAllData() {
    // Очищаем все данные из localStorage, кроме настроек темы и редактора
    const theme = localStorage.getItem('theme');
    const soundsEnabled = localStorage.getItem('sounds_enabled');
    const editorEnabled = localStorage.getItem('editor_enabled');
    ...
    localStorage.setItem('editor_enabled', editorEnabled !== 'false');
