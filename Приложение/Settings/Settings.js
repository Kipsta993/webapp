...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (testNotificationBtn) {
    testNotificationBtn.addEventListener('click', function() {
      ...
      // Запрашиваем разрешение на отправку уведомлений с использованием Service Worker
      if ('serviceWorker' in navigator) {
        // Проверяем, зарегистрирован ли Service Worker
        navigator.serviceWorker.ready.then(function(registration) {
          // Проверяем текущее состояние разрешения
          if (Notification.permission === "granted") {
            // Если разрешение уже дано, отправляем уведомление
            sendTestNotification();
          } else if (Notification.permission !== "denied") {
            // Если разрешение не запрашивалось или не определено, запрашиваем его
            Notification.requestPermission().then(function(permission) {
              if (permission === "granted") {
                // Если разрешение дано, отправляем уведомление
                sendTestNotification();
              } else {
                alert("Для отправки уведомлений необходимо разрешение. Пожалуйста, измените настройки в браузере.");
              }
            });
          } else {
            // Если разрешение было отклонено, показываем сообщение
            alert("Вы отклонили разрешение на уведомления. Пожалуйста, измените настройки в браузере.");
          }
        }).catch(function(error) {
          console.error('Ошибка при проверке готовности Service Worker:', error);
          alert('Не удалось инициализировать Service Worker для уведомлений. Пожалуйста, перезагрузите страницу и попробуйте снова.');
        });
      } else {
        // Если Service Worker не поддерживается, используем стандартный API
        if (Notification.permission === "granted") {
          // Если разрешение уже дано, отправляем уведомление
          sendTestNotification();
        } else if (Notification.permission !== "denied") {
  ...
  function sendTestNotification() {
    try {
      // Создаем иконку для уведомления (можно заменить на свою)
      const iconUrl = "https://cdn-icons-png.flaticon.com/512/1827/1827301.png";
      
      // Проверяем поддержку Service Worker
      if ('serviceWorker' in navigator && 'PushManager' in window) {
        // Используем Service Worker для отправки уведомлений
        navigator.serviceWorker.ready.then(function(registration) {
          registration.showNotification("Тестовое уведомление", {
            body: "Это тестовое уведомление от вашего приложения. Все работает корректно!",
            icon: iconUrl,
            vibrate: [200, 100, 200],
            timestamp: Date.now(),
          ...
          // Сохраняем информацию о том, что разрешение было получено
          localStorage.setItem('notificationsEnabled', 'true');
        }).catch(function(error) {
          console.error("Ошибка при отправке уведомления через Service Worker:", error);
          alert("Произошла ошибка при отправке уведомления через Service Worker: " + error.message);
        });
      } else {
        // Если Service Worker не поддерживается, пробуем использовать обычный API уведомлений
        ...
        notification.onclick = function() {
          // Фокусируемся на окне приложения при нажатии на уведомление
          window.focus();
          notification.close();
        };
