...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Функция для удаления всех классов стилей чекбоксов
  function clearAllCheckboxClasses() {
    // Удаляем старые классы стилей
    document.body.classList.remove('gold-checkboxes');
    
    // Удаляем классы для всех страниц
    const pages = ['all', 'daily', 'weekly', 'goals'];
    const styles = ['blue', 'purple', 'gold', 'ruby', 'standard']; // Добавляем новые стили, оставляем 'standard' для обратной совместимости
    const markStyles = ['empty', 'cross']; // Оставляем для обратной совместимости
    const shapeStyles = ['circle', 'square']; // Оставляем для обратной совместимости
    
    pages.forEach(page => {
      styles.forEach(style => {
        document.body.classList.remove(`${style}-checkboxes-${page}`);
      });
      
      markStyles.forEach(style => {
        document.body.classList.remove(`${style}-checkboxes-${page}`);
      });
      
      shapeStyles.forEach(shape => {
        document.body.classList.remove(`${shape}-checkboxes-${page}`);
      });
    });
  }
  ...
  function setCheckboxStyle(style, page) {
    ...
    pages.forEach(p => {
      ...
      document.body.classList.remove('blue-checkboxes-' + p);
      document.body.classList.remove('purple-checkboxes-' + p);
      document.body.classList.remove('gold-checkboxes-' + p);
      document.body.classList.remove('ruby-checkboxes-' + p);
      document.body.classList.remove('standard-checkboxes-' + p); // Для обратной совместимости
  ...
  function updateCheckboxSelectValues(page) {
    ...
    // Преобразуем старый стиль 'standard' в новый 'blue' для обратной совместимости
    const newSavedStyle = savedStyle === 'standard' ? 'blue' : savedStyle;
    ...
    if (styleSelect) {
      styleSelect.value = newSavedStyle;
    }

    if (markSelect) {
      markSelect.value = savedMarkStyle;
    }

    if (shapeSelect) {
      shapeSelect.value = savedShapeStyle;
    }
  }
  ...
  function applyCheckboxSettings() {
    ...
    pages.forEach(page => {
      ...
      // Преобразование старого значения 'standard' в новое 'blue'
      const newStyle = style === 'standard' ? 'blue' : style;
      
      // Применяем сохраненные стили
      if (newStyle !== 'default') {
        document.body.classList.add(newStyle + '-checkboxes-' + page);
      }
      
      // Следующие стили поддерживаются для обратной совместимости
    ...
    // Удаляем все классы 'all' для обратной совместимости
    document.body.classList.remove('blue-checkboxes-all');
    document.body.classList.remove('purple-checkboxes-all');
    document.body.classList.remove('gold-checkboxes-all');
    document.body.classList.remove('ruby-checkboxes-all');
    document.body.classList.remove('standard-checkboxes-all');
    document.body.classList.remove('empty-checkboxes-all');
    document.body.classList.remove('cross-checkboxes-all');
    document.body.classList.remove('circle-checkboxes-all');
    document.body.classList.remove('square-checkboxes-all');
  }

  // Функция сброса настроек чекбоксов
  function resetCheckboxSettings(page) {
    // Сбрасываем настройки для указанной страницы
    // Сбрасываем в localStorage
    localStorage.setItem('checkboxStyle-' + page, 'default');
    localStorage.setItem('checkboxMarkStyle-' + page, 'default');
    localStorage.setItem('checkboxShapeStyle-' + page, 'default');
    
    // Удаляем все классы стилей
    document.body.classList.remove('blue-checkboxes-' + page);
    document.body.classList.remove('purple-checkboxes-' + page);
    document.body.classList.remove('gold-checkboxes-' + page);
    document.body.classList.remove('ruby-checkboxes-' + page);
    document.body.classList.remove('standard-checkboxes-' + page); // Для обратной совместимости
    document.body.classList.remove('empty-checkboxes-' + page); // Для обратной совместимости
    document.body.classList.remove('cross-checkboxes-' + page); // Для обратной совместимости
    document.body.classList.remove('circle-checkboxes-' + page); // Для обратной совместимости
    document.body.classList.remove('square-checkboxes-' + page); // Для обратной совместимости
