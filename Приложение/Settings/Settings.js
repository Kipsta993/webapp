...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function sendTestNotification() {
    ...
    function sendStandardTestNotification() {
      try {
        console.log('Попытка отправки стандартного тестового уведомления');
        
        // Проверяем доступность ServiceWorkerRegistration
        if (navigator.serviceWorker && navigator.serviceWorker.ready) {
          // Используем ServiceWorkerRegistration.showNotification вместо new Notification
          navigator.serviceWorker.ready.then(registration => {
            console.log('Отправка тестового уведомления через ServiceWorkerRegistration.showNotification');
            ...
            registration.showNotification('Тестовое уведомление', {
              ...
              console.log('Стандартное тестовое уведомление успешно отправлено через ServiceWorkerRegistration');
              ...
              console.error('Ошибка при отправке fallback-уведомления через ServiceWorkerRegistration:', error);
              // В случае ошибки, пытаемся показать alert
              tryAlertTestNotification();
            });
          }).catch(error => {
            console.error('Ошибка при получении готового Service Worker для fallback:', error);
            tryAlertTestNotification();
          });
        } else {
    ...
    // Вспомогательная функция для отправки тестового уведомления через alert
    function tryAlertTestNotification() {
      try {
        alert('Тестовое уведомление');
        console.log('Отображено тестовое уведомление через alert');
      } catch (e) {
        console.error('Не удалось отправить даже alert-уведомление:', e);
      }
    }
  }
  ...
  function initDebugConsole() {
    ...
    versionElement.textContent = 'Ver 1.0';
    
    // Находим место для вставки версии
    const settingsContent = document.querySelector('.settings-content');
    if (settingsContent) {
      settingsContent.appendChild(versionElement);
    }
