...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function applyCheckboxSettings() {
    ...
    // Преобразование старого значения 'standard' в новое 'blue'
    const newAllStyle = allStyle === 'standard' ? 'blue' : allStyle;
    
    if (newAllStyle !== 'default') {
      document.body.classList.add(newAllStyle + '-checkboxes-all');
    }
  ...
  function resetCheckboxSettings(page) {
    // Получаем все страницы, которые нужно сбросить
    const pages = page === 'all' ? ['all', 'daily', 'weekly', 'goals'] : [page];
    
    // Для каждой страницы сбрасываем настройки
    pages.forEach(p => {
      // Сбрасываем в localStorage
      localStorage.setItem('checkboxStyle-' + p, 'default');
      localStorage.setItem('checkboxMarkStyle-' + p, 'default');
      localStorage.setItem('checkboxShapeStyle-' + p, 'default');
      ...
      document.body.classList.remove('ruby-checkboxes-' + p);
      document.body.classList.remove('standard-checkboxes-' + p); // Для обратной совместимости
      document.body.classList.remove('empty-checkboxes-' + p); // Для обратной совместимости
      document.body.classList.remove('cross-checkboxes-' + p); // Для обратной совместимости
      document.body.classList.remove('circle-checkboxes-' + p); // Для обратной совместимости
      document.body.classList.remove('square-checkboxes-' + p); // Для обратной совместимости
    });
