...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const timeSettingOptions = document.querySelectorAll('.time-setting-option');
  ...
  // Добавляем обработчики событий для опций настроек
  timeSettingOptions.forEach(option => {
    option.addEventListener('click', function(e) {
      // Если клик был не на чекбоксе или его лейбле
      if (!e.target.classList.contains('time-setting-checkbox') && 
          !e.target.tagName.toLowerCase() === 'label') {
        // Находим чекбокс внутри опции
        const checkbox = option.querySelector('.time-setting-checkbox');
