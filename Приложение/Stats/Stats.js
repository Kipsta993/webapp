...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Глобальные переменные для статистики целей и дневника
  let goalsStats = {
    total: 0,
    completed: 0,
    deleted: 0
  ...
  function showTasksPage() {
    ...
    // Останавливаем обновление времени при переходе на страницу задач
    stopTimeUpdate();
    
    // Загружаем статистику целей и дневника
    loadGoalsStats();
    loadDiaryStats();
  }
  ...
  window.StatsPage = {
    ...
    loadGoalsStats: loadGoalsStats,
    loadDiaryStats: loadDiaryStats
  };

  // Функция для сбора статистики целей
  function loadGoalsStats() {
    // Получаем текущие цели
    const goals = JSON.parse(localStorage.getItem('goals')) || [];
    
    // Получаем удаленные цели (создаем счетчик, если его нет)
    const deletedGoals = parseInt(localStorage.getItem('deletedGoalsCount')) || 0;
    ...
    goalsStats.completed = completedGoals;
    goalsStats.deleted = deletedGoals;
    
    updateGoalsStatsDisplay();
  }
  
  // Функция для обновления отображения статистики целей
  function updateGoalsStatsDisplay() {
    document.getElementById('goals-total').textContent = goalsStats.total;
    document.getElementById('goals-completed').textContent = goalsStats.completed;
    document.getElementById('goals-deleted').textContent = goalsStats.deleted;
  }
  
  // Функция для сбора статистики дневника
  function loadDiaryStats() {
    // Получаем текущие записи дневника
    const entries = JSON.parse(localStorage.getItem('diary-entries')) || [];
    
    // Получаем удаленные записи (создаем счетчик, если его нет)
    const deletedEntries = parseInt(localStorage.getItem('deletedEntriesCount')) || 0;
    
    // Обновляем объект статистики
    diaryStats.total = entries.length;
    diaryStats.deleted = deletedEntries;
    
    updateDiaryStatsDisplay();
  }
  
  // Функция для обновления отображения статистики дневника
  function updateDiaryStatsDisplay() {
    document.getElementById('diary-total').textContent = diaryStats.total;
    document.getElementById('diary-deleted').textContent = diaryStats.deleted;
  }
});
