...
function initStatsSlider() {
    ...
    function updateSlider() {
        ...
        slides.forEach((slide, index) => {
            ...
            if (index === currentIndex) {
                ...
                // Применяем анимацию к активному слайду
                slide.style.opacity = '0';
                slide.style.animation = 'fade-in-up 0.6s cubic-bezier(0.16, 1, 0.3, 1) forwards';
                
                // Анимируем элементы внутри активного слайда
    ...
    // Функция для анимации элементов внутри слайда
    function animateSlideElements(slide) {
        // Анимируем элементы stat-item
        const statItems = slide.querySelectorAll('.stat-item');
        statItems.forEach((item, index) => {
            item.style.opacity = '0';
            item.style.animation = 'scale-in 0.5s cubic-bezier(0.34, 1.56, 0.64, 1) forwards';
            item.style.animationDelay = `${0.3 + index * 0.1}s`;
        });
        
        // Анимируем элементы stat-row
        const statRows = slide.querySelectorAll('.stat-row');
        statRows.forEach((row, index) => {
            row.style.opacity = '0';
            row.style.animation = 'fade-in-up 0.4s cubic-bezier(0.16, 1, 0.3, 1) forwards';
    ...
    // Анимируем элементы управления слайдером
    const sliderControls = document.querySelector('.slider-controls');
    if (sliderControls) {
        sliderControls.style.opacity = '0';
        sliderControls.style.animation = 'fade-in-up 0.4s cubic-bezier(0.16, 1, 0.3, 1) forwards';
        sliderControls.style.animationDelay = '0.2s';
    }
}
