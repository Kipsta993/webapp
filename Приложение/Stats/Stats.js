...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function updateAppTime() {
    ...
    // Рассчитываем время в разных единицах без конвертации - просто сырые значения
    ...
    // Обновляем отображение всех единиц времени
  ...
  function updateWeekDisplay() {
    ...
    // Синхронизируем selectedWeekOffset с глобальным объектом
    if (window.StatsPage) {
      window.StatsPage.selectedWeekOffset = selectedWeekOffset;
    }
  ...
  weekPrevBtn.addEventListener('click', function() {
    selectedWeekOffset--;
    // Принудительно обновляем selectedWeekOffset в глобальном объекте
    if (window.StatsPage) {
      window.StatsPage.selectedWeekOffset = selectedWeekOffset;
    }
    updateWeekDisplay();
  ...
  function updateChartData(startDate, endDate) {
    ...
    // Константы для шкалы высоты с увеличенным контрастом
    const minHeight = 1; // Минимальная высота столбика (%) для нулевых значений
    const smallHeight = 10; // Высота для маленьких значений
    const maxHeight = 95; // Максимальная высота столбика (%)
    const contrastFactor = 1.5; // Фактор усиления контраста
    ...
    dayBars.forEach((dayBar, index) => {
      ...
      // Всегда показываем столбик, даже если день ещё не наступил
      dayColumns[index].style.visibility = 'visible';
      
      if (isFutureDay && selectedWeekOffset === 0) {
        // Для будущих дней текущей недели показываем нулевую активность
        dayBars[index].style.height = `${minHeight}%`; // Минимальная высота для видимости
        dayTimes[index].textContent = '0м';
      } else {
        // Для прошедших, текущих дней и всех дней других недель
        let height;
        
        if (dayTime <= 0) {
          // Для нулевых значений используем минимальную высоту
          height = minHeight;
        } else if (dayTime < 5) {
          // Для очень малых значений (меньше 5 минут) 
          height = smallHeight;
        } else {
          // Используем квадратичную функцию для усиления контраста
          // Это даст более заметную разницу между значениями
          const normalizedValue = Math.pow(dayTime / maxTime, contrastFactor);
          height = smallHeight + (normalizedValue * (maxHeight - smallHeight));
        }
  ...
  // Экспортируем общедоступные функции и переменные через глобальный объект
  window.StatsPage = {
  ...
  // Обновляем selectedWeekOffset в глобальном объекте при каждом изменении
  Object.defineProperty(window.StatsPage, 'selectedWeekOffset', {
    get: function() { return selectedWeekOffset; },
    set: function(value) { selectedWeekOffset = value; }
  });
  
  // Запускаем обновление таймера приложения
  if (currentPage === 1) {
    startTimeUpdate();
  }
});
