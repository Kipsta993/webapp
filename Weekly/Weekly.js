...
// Функция для обновления времени до следующего понедельника
function updateTimeUntilMonday() {
    const now = new Date();
    
    // Создаем дату следующего понедельника в 00:00 по московскому времени
    const nextMonday = new Date(now);
    const daysUntilMonday = (1 + 7 - now.getDay()) % 7; // 1 - понедельник, 0 - воскресенье
    
    if (daysUntilMonday === 0) {
        // Если сегодня понедельник, берем следующий понедельник
    ...
    // Форматируем строку
    const timeString = `${days}д ${hours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}`;
...
updateTimeUntilMonday(); // Вызываем сразу при загрузке
