document.addEventListener('DOMContentLoaded', function() {
    ...
    // Добавляем обработчики клика на элементы задач
    ...
    taskItems.forEach(taskItem => {
        taskItem.addEventListener('click', function(event) {
            ...
            if (!event.target.closest('input[type="checkbox"]')) {
                // Находим чекбокс внутри элемента задачи
                const checkbox = this.querySelector('input[type="checkbox"]');
                
                // Если чекбокс не отмечен и не отключен, отмечаем его
                if (checkbox && !checkbox.checked && !checkbox.disabled) {
                    checkbox.checked = true;
                    
                    // Вызываем событие change для запуска обработчика
...
function playCheckboxSound() {
    ...
    if (soundsEnabled) {
        // Ищем аудио-элемент в родительском документе (index.html)
        const sound = document.querySelector('#checkbox-sound') || 
                      parent.document.querySelector('#checkbox-sound');
